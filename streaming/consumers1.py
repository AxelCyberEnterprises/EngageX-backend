...
#                         media_path = os.path.join(tempfile.gettempdir(), f"{self.session_id}_{self.chunk_counter}_media.webm")
...
#                 # Combine buffered audio files.
#                 print(f"WS: Calling combine_audio_files with buffer: {self.audio_buffer}")
#                 combined_audio = self.combine_audio_files(self.audio_buffer)
#                 if combined_audio:
#                     print(f"WS: Combined audio file: {combined_audio}")
#                     executor.submit(self.run_sentiment_analysis, media_path, combined_audio)
#                 else:
#                     print("WS: Combined audio file creation failed.")
#             else:
...
#             return None
#         list_path = os.path.join(tempfile.gettempdir(), f"{self.session_id}_audio_list.txt")
#         with open(list_path, "w") as f:
#             for file_path in audio_files:
#                 f.write(f"file '{file_path}'\n")
#         combined_path = os.path.join(tempfile.gettempdir(), f"{self.session_id}_combined.mp3")
...
#     def run_sentiment_analysis(self, media_path, combined_audio_path):
...
#                         print("WS: Transcription Result:", transcript_text)
#                 else:
#                     print("WS: Combined audio file not found for transcription.")
#             except Exception as e:
#                 print(f"WS: Error during transcription: {e}")

#             if transcript_text:
#                 print("WS: Running analyze_results...")
#                 analysis_result = analyze_results(transcript_text, media_path, combined_audio_path)
#                 print("WS: Initial Analysis Result:", analysis_result)
#                 asyncio.create_task(self.save_chunk_data(media_path, analysis_result, transcript_text))
...
#                     "clarity": analysis_result.get('clarity'), # You might need to define how 'clarity' is represented
#                     "impact": analysis_result.get('impact'),
#                     "transformative_potential": analysis_result.get('transformative_potential'),
#                     "engagement_level": analysis_result.get('engagement'), # Assuming 'engagement' represents the level
#                 }
#                 print("WS: Sending Real-time Feedback:", feedback_data)
#                 asyncio.create_task(self.send(json.dumps(feedback_data)))

#             else:
#                 print("WS: No transcript available, skipping sentiment analysis and data saving.")
...
#             # Determine chunk start and end times (assuming 3-second chunks)
#             # start_time = (self.chunk_counter - 1) * 3
#             # end_time = self.chunk_counter * 3
#             # print(f"WS: Chunk start time: {start_time}, end time: {end_time}")
...
#                         'engagement': analysis_result.get('engagement', 0),
#                         'audience_emotion': analysis_result.get('audience_emotion', 0),
#                         'conviction': analysis_result.get('conviction', 0),
#                         'clarity': analysis_result.get('clarity', 0),
#                         'impact': analysis_result.get('impact', 0),
#                         'brevity': analysis_result.get('brevity', 0),
#                         'transformative_potential': analysis_result.get('transformative_potential', 0),
#                         'body_posture': analysis_result.get('body_posture', 0),
#                         'general_feedback_summary': analysis_result.get('general_feedback_summary', ''),
#                         'volume': analysis_result.get('volume'),
#                         'pitch_variability': analysis_result.get('pitch_variability'),
#                         'pace': analysis_result.get('pace'),
...
#             impact_scores = [item.impact for item in all_chunk_sentiments]
#             audience_engagement_scores = [item.engagement for item in all_chunk_sentiments]
...
#                 duration=timedelta(seconds=self.chunk_counter * 3) # Update the total duration
#             )
    ...
    async def analyze_chunk(self, media_path, audio_path):
        ...
        if transcript_text:
            ...
            feedback_data = {
                ...
                "audience_emotion": analysis_result.get('audience_emotion'),
                "conviction": analysis_result.get('conviction'),
                "clarity": analysis_result.get('clarity'),
                "impact": analysis_result.get('impact'),
                "brevity": analysis_result.get('brevity'),
                "transformative_potential": analysis_result.get('transformative_potential'),
                "body_posture": analysis_result.get('body_posture'),
                "general_feedback_summary": analysis_result.get('general_feedback_summary'),
                "volume": analysis_result.get('volume'),
                "pace": analysis_result.get('pace'),
                "pitch_variability": analysis_result.get('pitch_variability'),
