...
class LiveSessionConsumer(AsyncWebsocketConsumer):
    ...
    async def analyze_windowed_media(self, window_paths, latest_chunk_number):
        ...
        # Track window analysis attempt
        window_analysis_attempt = {
                ...
                while media_path not in self.media_path_to_chunk:
                    if time.time() - wait_start > 30:
                        if retry_count < max_retries:
                            retry_count += 1
                            print(f"WS: Retry {retry_count}/{max_retries} for chunk ID for {media_path}")
                            wait_start = time.time()
                            continue
                        else:
                            error_msg = f"Timeout waiting for chunk ID for {media_path} after {max_retries} retries"
                            print(f"WS: {error_msg}")
                            window_analysis_attempt['error'] = error_msg
                            self.window_analysis_details.append(window_analysis_attempt)
                            return
        ...
        window_analysis_attempt['chunk_numbers'] = window_chunk_numbers
        ...
        try:
            ...
            if combined_transcript_text.strip() and client and combined_audio_path and os.path.exists(combined_audio_path):
                ...
                try:
                    ...
                    if analysis_result is None or (isinstance(analysis_result, dict) and 'error' in analysis_result):
                        ...
                        window_analysis_attempt['error'] = error_message
                    ...
                    window_analysis_attempt['error'] = str(analysis_error)
                ...
                window_analysis_attempt['error'] = "Missing audio path for analysis"
                ...
                window_analysis_attempt['error'] = "OpenAI client not initialized"
            else:
                ...
                window_analysis_attempt['error'] = "Empty transcript"
            ...
            if analysis_result is not None:
                ...
                print(f"WS: Starting BACKGROUND analysis save for {window_chunk_number}")
                # Create a background task for saving the analysis
                ...
                self.background_chunk_save_tasks[f"analysis_{window_chunk_number}"] = save_task
                
                # Update window analysis details immediately
                window_analysis_attempt['saved'] = True
                self.window_analysis_details.append(window_analysis_attempt)
                print(f"WS: Window analysis #{self.total_window_analyses + 1} initiated for window ending with chunk {window_chunk_number}. Chunks used: {window_chunk_numbers}")
            else:
            ...
            traceback.print_exc()
            window_analysis_attempt['error'] = str(e)
            self.window_analysis_details.append(window_analysis_attempt)
        finally:
