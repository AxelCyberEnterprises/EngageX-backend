...
def score_pv(pitch_variability):
    ...
    if 60 <= pitch_variability <= 80:
        ...
        if pitch_variability < 15:
            rationale = (
                "Extremely low pitch variability; speech sounds robotic and unengaging."
            )
        else:
...
def analyze_posture(video_path):
    ...
    with lock:
        ...
        total_back_frames = (
            results_data["good_back_frames"] + results_data["bad_back_frames"]
        )
        total_back_time = total_back_frames / 30

        # calculate total duration
        total_neck_frames = (
            results_data["good_neck_frames"] + results_data["bad_neck_frames"]
        )
