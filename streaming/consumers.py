...
class LiveSessionConsumer(AsyncWebsocketConsumer):
    ...
    #                  # --- Wait for the background chunk save task for this specific chunk to complete ---
    #                  # This ensures the S3 upload and initial DB save are done before deleting files.
    #                  save_task = self.background_chunk_save_tasks.get(oldest_media_path)

    #                  if save_task:
    #                      print(f"WS: Waiting for background save task for {oldest_media_path} to complete...")
    #                      try:
    #                          # Wait for the specific task to finish (with a timeout to avoid infinite waits)
    #                          # Increased timeout slightly, though S3 upload times suggest it might need more
    #                          await asyncio.wait_for(save_task, timeout=90.0) # Wait up to 90 seconds
    #                          print(f"WS: Background save task for {oldest_media_path} completed.")
    #                      except asyncio.TimeoutError:
    #                          print(f"WS: Timeout waiting for background save task for {oldest_media_path}. Skipping cleanup of this chunk for now.")
    #                          # Skip cleanup for this chunk in this iteration; it might be cleaned up later
    #                          # Break the while loop to avoid blocking further cleanup attempts for other chunks
    #                          break

    #                      except Exception as task_error:
    #                          # This handles exceptions within the background save task itself (e.g., S3 upload failure, DB error)
    #                          print(f"WS: Background save task for {oldest_media_path} failed with error: {task_error}. Proceeding with cleanup as task is done.")
    #                          # The task failed, but it's finished. We can proceed with cleanup.

    #                  else:
    #                       # This case might happen if cleanup runs significantly later and the task finished/failed and removed itself,
    #                       # or if process_media_chunk had an error before starting the task.
    #                       print(f"WS: No background save task found for {oldest_media_path}. Assuming it finished or wasn't started. Proceeding with cleanup.")
    ...
    #                  # The background_chunk_save_tasks entry is removed within _complete_chunk_save_in_background.
...
# Modify analyze_windowed_media method
    ...
    async def analyze_windowed_media(self, window_paths, latest_chunk_number):
        ...
        try:
            ...
            if analysis_result is not None:
                ...
                wait_timeout = 15.0 # Increased timeout to wait for the task to appear/complete
